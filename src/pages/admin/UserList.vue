...
<template>
    ...
    <input type="text" v-model='currentUsername'/><div class="button-control"><base-button mode="flytrap" @click="changeUsername">Submit</base-button>
    </div></div>
    </form>
    </base-dialog>
    <base-card>
    <div class="containing">
        ...
        <ul v-if="allUsers">
            <li v-for="user in allUsers" :key="user.userNumber"><span class='names'>{{user.username}}</span> - <span class="emails">{{user.email}}</span> <base-button :mode="user.canNavigate ? 'wallflower' : 'flytrap'" @click="changeNavigation(user.username, user.canNavigate, user.email, user.userNumber)"><img src='./../../assets/deleteUser.png' class="deletes"></base-button>
                <base-button mode="wallflower" @click="toggleDialog(user.username, user.email, user.canNavigate, user.userNumber)"><img src="./../../assets/edit.png" class="edits"></base-button></li>
        </ul>
        <p v-else>Loading...</p>
        </div>
