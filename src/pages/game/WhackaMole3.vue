<template>
    ...
    methods: {
        ...
        reInit(){
            ...
            let intervalId = setInterval(this.handleRandomizer, 500);
            let intervalId2 = setInterval(this.handleRandomizer, 550);
            setTimeout(()=>{clearInterval(intervalId)
                document.querySelector('.boton-exit').disabled = false;
    }, 10000);
    setTimeout(()=>{clearInterval(intervalId2)
                document.querySelector('.boton-exit').disabled = false;
    }, 10000);
        },
        ...
        handleRandomizer(){
            if(!this.randomNo){ 
            this.randomNo = Math.floor(Math.random()*7)+1;
            }else{
               ...
               this.randomClass = Math.floor(Math.random()*7)+1;
                ...
                this.randomNo2 = Math.floor(Math.random()*(15-8+1))+8;

            }else{
                document.querySelector(`#cell${this.randomNo2}`).removeEventListener('click', this.handleClick);
               document.querySelector(`#cell${this.randomNo2}`).classList.remove(`InUse${this.randomClass}`);
               this.randomNo2 = Math.floor(Math.random()*9)+1;
               this.randomClass = Math.floor(Math.random()*7)+1;
    
            }
            let randomCell = document.querySelector(`#cell${this.randomNo}`);
            let randomCell2 = document.querySelector(`#cell${this.randomNo2}`);
            ...
            randomCell2.classList.add(`InUse${this.randomClass}`);
        },
